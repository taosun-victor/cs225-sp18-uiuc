...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while( (singleQueue.size() + mergeQueue.size()) > 1) {
        TreeNode * small_1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode * small_2 = removeSmallest(singleQueue, mergeQueue);
      
        TreeNode * parent = new TreeNode( (small_1->freq).getFrequency() + 
                                          (small_2->freq).getFrequency() );
        parent->left = small_1;
