...
size_t binary_search(const std::vector<T>& elements, const C& val){

    size_t low = 0;
    size_t high = elements.size() - 1;

    while (!(low > high)){
        size_t midpt = (low + high)/2;
        if (val == elements[midpt])
            return midpt;
        if (val > elements[midpt] && val < elements[midpt + 1])
            return (midpt + 1);
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return binary_search(elements, val);
