...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         */
         ...
         string customer_url_combine = customer + url;
         time_t date = ll.date;
         ...
         if (!whenVisitedTable.keyExists(customer_url_combine))
             whenVisitedTable.insert(customer_url_combine, date);
         else{
             if (date >  whenVisitedTable[customer_url_combine])
                 whenVisitedTable[customer_url_combine] = date;
         }
                                  ...
                                  const string& url) const
{
    ...
    string customer_url_combine = customer + url;
    if (whenVisitedTable.keyExists(customer_url_combine))
        return whenVisitedTable.find(customer_url_combine);
    else
        return time_t();
