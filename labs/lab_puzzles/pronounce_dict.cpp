...
{
    /* Your code goes here! */
    //std::cout << "enter homophones" << std::endl;

    std::string word1_upper = word1;
    std::transform(word1_upper.begin(), word1_upper.end(), word1_upper.begin(), toupper);

    std::string word2_upper = word2;
    std::transform(word2_upper.begin(), word2_upper.end(), word2_upper.begin(), toupper);

    //std::cout << word1_upper << " " << word2_upper << std::endl;

    auto lookup1 = dict.find(word1_upper);
    auto lookup2 = dict.find(word2_upper);

    //std::cout << "hhh" << std::endl;

    if (lookup1 != dict.end() && lookup2 != dict.end()){
        if (lookup1->second == lookup2->second) return true;
        else return false;
    }
    else{
        return false;
    ...
    //if (dict[word1_upper] == dict[word2_upper]) return true;
    //else return false;
}
