...
class BinaryTree
{
  ...
  private:
    ...
    T minTree(Node * subRoot) const;
  
    T maxTree(Node * subRoot) const;

    bool isOrderedR(Node * subRoot) const;

    void insert(Node*& node, const T& elem, bool sorted);

    void printPathsRecur(Node * subRoot, vector<vector<T>> &paths, T path[], int pathLen) const;

    void storePath(vector<vector<T>> &paths, T path[], int pathLen) const;

    int sumDistances(Node * subRoot, int pathLen) const;
