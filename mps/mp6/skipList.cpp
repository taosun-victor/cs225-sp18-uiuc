...
void SkipList::insert(int key, HSLAPixel value) {
    ...
    for(int i = listHeight_; i < max(this->listHeight_, newNodeLevel); i++) {
        ...
        head_->nodePointers[head_->nodePointers.size()-1].next = tail_;
        tail_->nodePointers[tail_->nodePointers.size()-1].prev = head_;
    }
...
SkipNode * SkipList::findR(int key) {
    SkipNode * fr = findRHelper(key, head_->nodePointers.size() - 1, head_);
    return fr;
}
