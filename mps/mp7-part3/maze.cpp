...
PNG * SquareMaze::drawCreativeMaze() const {
    ...
    int radius_block = 10;
    double angle = 2*PI/width_;
    cout << "angle: " << angle << endl;
    int dim = (height_+1) * radius_block *2;


    PNG * maze_png = new PNG(dim, dim);
    int center_x = dim/2;
    int center_y = dim/2;

    // cap the inner line
    //int inner_block = radius_block * angle;
    for (int i=center_x-radius_block-1; i<=center_x+radius_block+1; i++){
        for (int j=center_x-radius_block-1; j<=center_x+radius_block+1; j++){
            if (distance(i,j,center_x, center_y) > radius_block-0.5 &&
                distance(i,j,center_x, center_y) < radius_block+0.5   ){
    ...
    for (int i=0; i<size_; i++){
        ...
        if (rightWall_[i] == true){
            double curr_angle = angle * maze_x;
            double curr_radius = radius_block * (maze_y+1);

            cout << "curr-angle: " << curr_angle;
            cout << "curr-radius: " << curr_radius;
            cout << endl;

            for (int j=0; j<=radius_block; j++){
                int curr_x = int( (curr_radius+j) * cos(curr_angle));
                int curr_y = int( (curr_radius+j) * sin(curr_angle));
...
PNG * SquareMaze::drawCreativeMazeWithSolution(){
    int radius_block = 10;
    int dim = (height_+1) * radius_block;
    PNG * maze_png = new PNG(dim, dim);
    return maze_png;
}
