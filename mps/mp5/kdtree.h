...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(vector<Point<Dim>> & Points, int dim, int left, int right);
    
    Point<Dim> kthSmallest(vector<Point<Dim>> & Points, int dim, int left, int right, int k);

    KDTreeNode * vectorToBST(vector<Point<Dim>> & Points, int level, int left, int right);
    ...
    double dist_plane(const Point<Dim> &first, const Point<Dim> &second, int level) const;

    Point<Dim> search_subtree_topdown(const KDTreeNode * subRoot, Point<Dim> query, Point<Dim> curr_best, double dist_min, int level) const;

    Point<Dim> search_subtree_parent(const KDTreeNode * subRoot, Point<Dim> query, int level, Point<Dim> & curr_best) const;
